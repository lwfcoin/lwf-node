  ...
  getForgers: function (params) {
    var sql = [
      ...
      'delegates AS (SELECT row_number() OVER (ORDER BY vote DESC, m."publicKey" ASC)::int AS rank,',
        ...
        'COALESCE(v.voters_cnt, 0) AS voters_cnt,',
        't.timestamp AS register_timestamp',
      'FROM (SELECT DISTINCT ON (username) * FROM delegates) d',
      'LEFT JOIN mem_accounts m ON d.username = m.username',
      'LEFT JOIN trs t ON d."transactionId" = t.id',
      'LEFT JOIN (SELECT "dependentId", COUNT(1)::int AS voters_cnt from mem_accounts2delegates GROUP BY "dependentId") v ON v."dependentId" = ENCODE(m."publicKey", \'hex\')',
      'WHERE m."isDelegate" = 1',
      'ORDER BY ' + [params.sortField, params.sortMethod].join(' ') + ')',
      'SELECT * FROM delegates LIMIT ${limit} OFFSET ${offset}'
    ].join(' ');

    return pgp.as.format(sql, params);
  },

  // getLastBlockByGeneratorPublicKey: function() {
  //   var sql = [
  //     'WITH',
  //     'lastBlocks AS (SELECT row_number() OVER (ORDER BY b."height" DESC)::int AS rn,',
  //       'b.*',
  //     'FROM blocks b',
  //     'WHERE b."generatorPublicKey" = DECODE(${generatorPublicKey}, \'hex\'))',
  //     'SELECT * FROM lastBlocks WHERE lastBlocks."rn" = 1'
  //   ].join(' ');
    
  //   return sql;
  // },

  getLastBlockByGeneratorPublicKey:  'SELECT * FROM blocks b WHERE b."generatorPublicKey" = DECODE(${generatorPublicKey}, \'hex\') ORDER BY b."height" DESC LIMIT 1;',
