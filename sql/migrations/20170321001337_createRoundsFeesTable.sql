...
CREATE FUNCTION rounds_fees_init() RETURNS void LANGUAGE PLPGSQL AS $$
	...
	BEGIN
		...
		FOR row IN
			SELECT
				-- Round number
				CEIL(height / 201::float)::int AS round
			FROM blocks
			-- Perform only for rounds that are completed and not present in 'rounds_fees'
			WHERE height % 201 = 0 AND height NOT IN (SELECT height FROM rounds_fees)
			-- Group by round
			GROUP BY CEIL(height / 201::float)::int
			-- Order by round
			ORDER BY CEIL(height / 201::float)::int ASC
		LOOP
			WITH
				-- Selecting all blocks of round
				round AS (SELECT timestamp, height, "generatorPublicKey", "totalFee" FROM blocks WHERE CEIL(height / 201::float)::int = row.round),
				-- Calculating total fees of round
				fees AS (SELECT SUM("totalFee") AS total, FLOOR(SUM("totalFee") / 201) AS single FROM round),
			...
			INSERT INTO rounds_fees
				SELECT
					...
					(fees.single + (CASE WHEN last.pk = round."generatorPublicKey" AND last.timestamp = round.timestamp THEN (fees.total - fees.single * 201) ELSE 0 END)) AS fees,
					-- Delegate public key
...
CREATE INDEX IF NOT EXISTS "rounds_fees_round" ON "rounds_fees" ((CEIL(height / 201::float)::int));
...
CREATE FUNCTION round_fees_delete() RETURNS TRIGGER LANGUAGE PLPGSQL AS $$
	BEGIN
		DELETE FROM rounds_fees WHERE CEIL(height / 201::float)::int = (CEIL(OLD.height / 201::float)::int);
	RETURN NULL;
END $$;
...
CREATE FUNCTION round_fees_insert() RETURNS TRIGGER LANGUAGE PLPGSQL AS $$
	BEGIN
		WITH
			-- Selecting all blocks of round
			round AS (SELECT timestamp, height, "generatorPublicKey", "totalFee" FROM blocks WHERE CEIL(height / 201::float)::int = CEIL(NEW.height / 201::float)::int),
			-- Calculating total fees of round
			fees AS (SELECT SUM("totalFee") AS total, FLOOR(SUM("totalFee") / 201) AS single FROM round),
			-- Get last delegate and timestamp of round's last block
			last AS (SELECT "generatorPublicKey" AS pk, timestamp FROM round ORDER BY height DESC LIMIT 1)
